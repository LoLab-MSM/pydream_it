...
class DreamParam(object):
    ...
    def __call__(self, parameter, dist=uniform, loc=None, width=6):
    ...
    def fancy_index(self, parameters):
        idxs = list()
    ...
    def sampled_params(self):
        ...
        return [SampledParam(self.parms[name][0], loc=self.parms[name][1], scale=self.parms[name][2]) for name in self.keys()]
